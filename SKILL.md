...
agent_created: true
license: MIT
display_name: "新媒体运营工作台 v24 复现"
display_name_en: "New-Media Ops Workbench v24 Repro"
disable: false
---

# 新媒体运营工作台 · v24 复现 Workflow

## 0. 这是什么 / 何时用

要从零复现 v24，严格按本工作流的顺序执行。本工作流提炼自「新媒体运营工作台」从原型到 v24 的**已验证、去踩坑**路径。

在以下场景使用本 skill：
- 从空白目录重建整个工作台，要求结果等价于已交付的 v24。
- 理解模块开发顺序、各版本里程碑的关键决策。
- 在新机器 / 新会话里继续维护或扩展该工作台。

> 复现时直接复制 `references/v24-source/` 的字节级源码归档（与 v24 完全一致），再按本工作流核对关键决策，无需重写代码。最快路径见第 11 节。
...
新建项目根目录，按以下结构落地（与 `references/v24-source/` 一致）：
...
- `sw.js`：`CACHE` 常量版本化（当前 `ops-v24`），`ASSETS` 列表带 `?v=24`；改前端资源后必须 bump 此版本号，手机才会拉到新版。
...
### 步骤 7 · v20→v24 功能迭代（数据管理 / 账号编辑删除 / 侧栏重构 / 批量删除）
- 在稳定版 v19 之上，按真实运营需求补齐数据管理与编辑删除能力。
- **v20 · 设置页「彻底清空数据」+ 账号编辑/删除 + 密码备注 + 级联删除**
  - 设置页新增真正生效的「清空数据」：清除全部用户数据（`accounts/memos/todos/reminders/topics/metrics`）+ 重置 `profile`，同步清空 localStorage（开启云端同步时推送空状态到后端）；通知/同步开关保留。
  - 账号模型新增 `password` / `note`（选填）；账号卡片点击进入编辑，弹窗完整回填、保存为按 `id` 更新（非新增）。
  - 删除账号**级联清除**其下全部关联数据（备忘/待办/提醒/选题/指标）；卡片右上角常显 🗑 删除图标（v21）。
- **v22 · 编辑弹窗打磨**：`openModal('account',id)` 完整回填现有字段，底部显示「🗑 删除此账号」，保存走 `updateAccount` 而非新增。
- **v24 · 侧栏重构 + 模块内批量删除**
  - 侧栏分区最终结构：**模块**（多账号面板/备忘录/待办/提醒/**选题素材库**）+ **数据分析**（发布日历/数据看板/周月报）；「选题素材库」由数据分析区移入模块区。
  - 移除模块入口级批量删除，改为**模块内卡片批量多选删除**：进入含卡片的模块→点「批量」→卡片左上角复选框多选→「删除」二次确认→一键删除；切换模块/视图自动退出批量模式；账号卡片删除仍级联。
- **验收**：设置页清空后所有模块回到空状态；账号编辑保存不产生重复卡片；批量删除 Toast 提示正确；侧栏选题入口位于「模块」区。

---

## 4. 版本里程碑 v1 → v24（关键决策表）
...
| v20.0.0 | 数据管理 | 设置页清空数据；账号卡片编辑/删除；账号密码/备注字段；级联删除 | `clearData`；`account.password/note`；`cascadeDelete` |
| v21.0.0 | 编辑体验 | 账号卡片右上角常显 🗑 删除图标 | 快速删除入口 |
| v22.0.0 | 编辑打磨 | 账号编辑弹窗完整回填、保存为更新、底部删除入口 | `openModal('account')` 回填 |
| v24.0.0 | 侧栏/批量 | 选题素材库归入「模块」区；模块内卡片批量多选删除 | 侧栏分区重构；`batchDelete` |
...
**数据模型（务必一致）**
- `account`: `{id, name, platform, icon, status:'活跃|孵化中|休眠', goal, password?, note?, createdAt}`（`password`/`note` 为 v20 选填；删除账号级联清除其下全部关联数据）
...
5. **资源改动必 bump `sw.js` 的 `CACHE`**（如 `ops-v24`→`ops-v25`）与 `index.html`/`app.js` 的 `?v=N`、`app.js` 的 `APP_VERSION`，手机才会拉到新版。
...
**8 大模块**
1. **多账号面板**：账号矩阵、健康分、待办完成度进度条；卡片点击进入编辑（含密码/备注字段）、右上角常显 🗑 删除图标；点侧栏账号→仅显该账号；点「全部账号」→合并。
...
6. **选题素材库（位于「模块」区）**：搜索、状态过滤、点击编辑/删除、置顶优先；多账号面板显各账号选题数。
...
- 设置中心 ⚙：提醒通知开关+权限；云端同步开关+后端地址+测试连接（默认关）；导出/导入/清空；设置页新增「彻底清空数据」（清除全部用户数据 + 重置 profile，同步清空 localStorage）。
- 模块内「批量」删除：进入含卡片的模块→点「批量」→卡片左上角复选框多选→「删除」二次确认→一键清除；切换模块/视图自动退出批量模式；账号卡片删除仍级联。
...
1. 复制 `references/v24-source/` 整目录到新项目根。
...
## 附：产品描述与使用说明（对外文档·图文版）

> 本 skill 附「描述（使用说明）」文档，内容与工作台**完全一致**，涵盖产品目标、功能演进（v1→v24）、8 大模块图文用法、部署与移动端验收，以及完整使用指南。图文版已随截图打包进 `references/v24-source/docs/screenshots/`，图片链接有效。

> 文件位置（按需读取，避免 SKILL.md 臃肿）：
> - `references/v24-source/docs/使用说明.md`（图文版功能使用介绍，含 15 张界面截图）
> - `references/v24-source/docs/开发日志-对外版.md`（产品目标与功能演进时间线）
> - `references/v24-source/docs/更新记录-最终版-v24.md`（v20→v24 变更明细与 6 张截图）
